...
void NotificationEngine::apply( Framebuffer &fb ) const
{
  ...
  if ( message.empty() && (!time_expired) ) {
    ...
    swprintf( tmp, 128, L"mosh: %s%S", message.c_str(), keystroke_str );
  } else {
    swprintf( tmp, 128, L"mosh: %s (%S without %S.)%S", message.c_str(),
	      human_readable_duration( static_cast<int>( time_elapsed ),
				       "s" ).c_str(),
...
int NotificationEngine::wait_time( void ) const
{
  ...
  next_expiry = min( next_expiry, message_expiration - now );
  ...
  if ( need_countup( now ) ) {
    ...
    next_expiry = min( next_expiry, countup_interval );
  }
