...
struct winsize {
	int ws_col;
	int ws_row;
	winsize() : ws_col(0), ws_row(0) { }
};

struct ITerminal {
	virtual winsize GetSize() = 0;
	virtual void SetSize(const winsize& newsize) = 0;
	virtual void Clear() = 0;
	virtual void Update(Terminal::Framebuffer*) = 0;
...
public:
      ...
      clean_shutdown( false )
  {
    ...
    bool process_user_input( int charCode );

  void init( ITerminal* iTerminal );
  void shutdown( void );
  void main( void );
  bool oneloop(void);
