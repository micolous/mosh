...
void STMClient::init( ITerminal* iTerminal )
{
	if (iTerminal == NULL)
	{
		perror("ITerminal interface null");
	}

	this->iTerminal = iTerminal;
	iTerminal->Clear();

	window_size = iTerminal->GetSize();
...
bool STMClient::process_user_input( int charCode )
{
	byte buf[5];
	...
	{
		bytes_read = 2;
		buf[0] = 0xc0 + (charCode >> 6);
		buf[1] = 0x80 + (charCode & 0x3f);
	}
	else if (charCode < 0x10000)
	{
		bytes_read = 3;
		buf[0] = 0xe0 + (charCode >> 12);
		buf[1] = 0x80 + ((charCode >> 6) & 0x3f);
		buf[2] = 0x80 + (charCode & 0x3f);
	}

//  /* fill buffer if possible */
//  ssize_t bytes_read = _read( fd, buf, buf_size );
//  if ( bytes_read == 0 ) { /* EOF */
//    return false;
...
bool STMClient::oneloop( void )
{
	...
	try {
		...
		if ( network_ready_to_read ) {
			if ( !process_network_input() ) { return false; }
		}
